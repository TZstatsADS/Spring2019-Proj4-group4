...
word <- unlist(lapply(list,bigram)) #778458 elements
LB <- data.frame(table(word))
```

### 3.1.2 Obtain ocr data and extract the features
```{r}
tesseract_if_clean <- read.csv("../output/ocr_output.csv")

feature_names <- paste("feature", 1:13)
features <- matrix(nrow =nrow(tesseract_if_clean),  ncol = 13)
for(i in 1:nrow(tesseract_if_clean)){
  features[i,] <- tesseract_if_clean$tokens[i] %>% as.character() %>% findfeatures() 
}

colnames(features) <- feature_names
features <- cbind(tesseract_if_clean, features)

save(features, file="../output/features.RData")
write.csv(features, file="../output/features.csv")
